...
import { articles as fallbackArticles, siteConfig } from "@/lib/data";
...
export default function ArticleContent() {
  ...
  const [relatedArticles, setRelatedArticles] = useState<FirestoreArticle[]>([]);
  const [popularArticles, setPopularArticles] = useState<FirestoreArticle[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchData() {
      try {
        const articlesSnap = await getDocs(query(collection(db, "articles"), where("slug", "==", slug)));
        if (!articlesSnap.empty) {
          const doc = articlesSnap.docs[0];
          const found = { id: doc.id, ...doc.data() } as FirestoreArticle;
          setArticle(found);

          if (found.categorySlug) {
            const relatedSnap = await getDocs(
              query(collection(db, "articles"), where("categorySlug", "==", found.categorySlug), fsLimit(5))
            );
            setRelatedArticles(
              relatedSnap.docs
                .map((d) => ({ id: d.id, ...d.data() } as FirestoreArticle))
          ...
          const popularSnap = await getDocs(
            query(collection(db, "articles"), orderBy("views", "desc"), fsLimit(5))
          );
          setPopularArticles(popularSnap.docs.map((d) => ({ id: d.id, ...d.data() } as FirestoreArticle)));
        } else {
          ...
          if (fb) {
            setArticle({
              ...
              tags: fb.tags,
              metaTitle: fb.seoTitle,
              metaDescription: fb.metaDescription,
              pros: fb.pros,
              cons: fb.cons,
              faqs: fb.faqs,
              tableOfContents: fb.tableOfContents,
              rating: fb.rating,
            });
          }
        }
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-pulse space-y-4">
          <div className="h-4 bg-secondary-200 rounded w-64" />
          <div className="h-8 bg-secondary-200 rounded w-96" />
  ...
  if (!article) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        <h1 className="text-2xl font-bold text-secondary-900 mb-2">Article Not Found</h1>
        <p className="text-secondary-500 mb-6">The article you are looking for does not exist.</p>
        <Link href="/" className="text-primary-500 hover:text-primary-600 font-medium">Go Home →</Link>
      </div>
    );
  }

  const authorName = typeof article.author === "string" ? article.author : "Admin";
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        datePublished: article.createdAt ? new Date(article.createdAt.seconds * 1000).toISOString() : "",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Breadcrumbs items={[
          { label: "Home", href: "/" },
          { label: article.category || "General", href: `/category/${article.categorySlug || ""}` },
          { label: article.title },
        ]} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <article className="lg:col-span-8">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4 pb-6 border-b border-secondary-100">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-100 to-accent-100 flex items-center justify-center">
                    <span className="text-sm font-bold text-primary-500">{authorInitial}</span>
                  </div>
                  <div>
                    <div className="text-sm font-medium text-secondary-900">{authorName}</div>
                    <div className="text-xs text-secondary-400">{article.readTime || 5} min read · {(article.views || 0).toLocaleString()} views</div>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <span className="text-xs text-secondary-400 mr-1">Share:</span>
                  <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(`${siteConfig.url}/article/${article.slug}`)}&text=${encodeURIComponent(article.title)}`} target="_blank" rel="noopener noreferrer" className="p-2 text-secondary-400 hover:text-[#1DA1F2] hover:bg-secondary-50 rounded-lg transition-colors">
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                  </a>
                  <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`${siteConfig.url}/article/${article.slug}`)}`} target="_blank" rel="noopener noreferrer" className="p-2 text-secondary-400 hover:text-[#1877F2] hover:bg-secondary-50 rounded-lg transition-colors">
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  </a>
                  <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(`${siteConfig.url}/article/${article.slug}`)}&title=${encodeURIComponent(article.title)}`} target="_blank" rel="noopener noreferrer" className="p-2 text-secondary-400 hover:text-[#0A66C2] hover:bg-secondary-50 rounded-lg transition-colors">
            ...
            {article.tags && article.tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-8 pb-8 border-b border-secondary-100">
                {article.tags.map((tag) => (
                  <Link key={tag} href={`/search?q=${encodeURIComponent(tag)}`} className="px-3 py-1.5 bg-secondary-50 text-secondary-600 text-xs font-medium rounded-full hover:bg-primary-50 hover:text-primary-600 transition-colors">
                    {tag}
                  </Link>
                ))}
              </div>
            ...
            <div className="mb-10">
              <AuthorBox author={{ name: authorName, bio: "" }} />
            </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-gradient-to-br from-primary-500 to-primary-700 rounded-xl p-6 text-white">
              ...
              <Link href="/tools" className="inline-flex items-center gap-2 bg-white text-primary-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary-50 transition-colors">
                View All Tools →
              </Link>
            </div>
                ...
                <div className="space-y-3">
                  {popularArticles.map((a) => (
                    <Link key={a.id} href={`/article/${a.slug}`} className="block group">
                      ...
                      <span className="text-xs text-secondary-400">{a.readTime || 5} min read</span>
                    </Link>
                  ))}
                </div>
              </div>
